...
import com.uptang.cloud.core.util.CacheKeys;
import com.uptang.cloud.core.util.CollectionUtils;
import com.uptang.cloud.core.util.NumberUtils;
import com.uptang.cloud.core.util.SerializationUtils;
...
public class UserTokenUtils {
    ...
    private final Cache<Long, UserContext> userContextCache;
     ...
     * @return 可传递的授权令牌
     */
    public String generateToken(Long userId, UserTypeEnum userType, String host) {
     ...
     * @return 用户ID
     */
    public Long getUserIdDirectly(String token) {
     ...
     * @return 用户ID
     */
    public Long getUserIdDirectly(String token, boolean sendToQueue) {
        if (StringUtils.isBlank(token)) {
            return null;
        }

        Long userId = this.tokenCache.getIfPresent(token);
    ...
    public UserContext getUserContext(String token) {
        ...
        Long userId = getUserIdDirectly(token, false);
        ...
        String cachedUserContext = redisTemplate.opsForValue().get(getCacheKey(userId));
    ...
    private void tokenVerify() {
        ...
        // 将 Token 转换成 UserID
        List<Long> userIds = tokens.stream()
        ...
        // 从 Redis 中获取数据
        List<String> contexts = redisTemplate.opsForValue()
                .multiGet(userIds.stream().map(UserTokenUtils::getCacheKey).collect(Collectors.toList()));
    ...
    public static String getCacheKey(long userId) {
        return CacheKeys.getKey(String.join(":", "user", "sso", "context"), userId);
    }
}
